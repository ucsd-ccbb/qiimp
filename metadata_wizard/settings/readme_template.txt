...
1) Start on the 'Metadata' tab:
    a) Entering your sample names, either one by one or via copy-paste.
Note: Sample names may only contain alphanumeric characters (A-Z,a-z,0-9)
and the period character (.) .
    b) As you enter each sample name, fields (columns) with only one
permitted value will automatically be completed based on the information
you supplied in the web form.
    c) Fill in the remaining information for each metadata field (column);
